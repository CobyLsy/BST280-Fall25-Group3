...
ggplot(pca_df, aes(PC1, PC2, color = Type)) +
  ...
  labs(title = "PCA of LUAD vs LUSC",
       x = paste0("PC1"),
       y = paste0("PC2"))
```
...
```{r}
tab <- table(TrueType = pca_df$Type,
             Cluster = pca_df$KmeansCluster)
tab
```
...
ggplot(pca_df, aes(PC1, PC2, color = Type)) +
  geom_point(size = 1.5, alpha = 0.8) +
  theme_minimal(base_size = 16) +
  labs(title = "PCA of LUAD vs LUSC (Top 500)",
       x = paste0("PC1"),
       y = paste0("PC2"))
```
...
ggplot(pca_df, aes(PC1, PC2, color = KmeansCluster)) +
  geom_point(size = 1.5, alpha = 0.8) +
  theme_minimal(base_size = 16) +
  labs(title = "K-means Clustering on PCA (Top 500)",
       color = "Cluster")
```
...
```
```{r}
all_deg$SigGroup <- "Not significant"

all_deg$SigGroup[all_deg$log2FoldChange > 2] <- "LUAD up"
all_deg$SigGroup[all_deg$log2FoldChange < -2] <- "LUSC up"

ggplot(all_deg, aes(x = log2FoldChange, y = -log10(padj), color = SigGroup)) +
  ...
  labs(
    title = "Volcano Plot: LUAD vs LUSC",
    x = "log2 Fold Change (LUAD / LUSC)",
...
sel_samples <- c(luad_samples, lusc_samples)
...
pheatmap(
  ...
  show_rownames = TRUE,
  fontsize_row = 4,
  main = "Top 50 DEGs in LUAD vs LUSC samples",
)
```
...
ggplot(pca_deg_df, aes(PC1, PC2, color = KmeansCluster)) +
  ...
  theme_minimal(base_size = 14) +
  labs(
    title = "K-means Clustering on PCA of DE Genes",
    color = "Cluster"
  )
```
...
- Ranked all DE genes
- Perform GSEA keeping only high significantly enriched pathways (adjusted p-value > 0.05)
- Evaluate highest positive and negative normalized enrichment scores (NES)

## Match ensembl ID with entrez ID for all DE genes
```{r}
library(biomaRt)

#Get the entrez IDs using biomaRt
ensembl <- useMart("ensembl",dataset="hsapiens_gene_ensembl",host = "https://www.ensembl.org")

#Remove the version of the ensembl ID
ens_genes <- sub("\\..*", "", deg_genes)

#Match each ensembl ID with the corresponding entrez ID
...
## Create lists of most differntialy expressed genes
```{r}
# Define the set of differentially expressed genes for each tumor that are above the threshold
up_LUSC <- rownames(deg)[which(deg$log2FoldChange > 2 & deg$padj < 0.05)]
up_LUSC <- sub("\\..*", "", up_LUSC)
up_LUSC_ID <- annot[match(up_LUSC, annot$ensembl_gene_id), "entrezgene_id"]

up_LUAD <- rownames(deg)[which(deg$log2FoldChange < -2 & deg$padj < 0.05)]
up_LUAD <- sub("\\..*", "", up_LUAD)
up_LUAD_ID <- annot[match(up_LUAD, annot$ensembl_gene_id), "entrezgene_id"]
```

## Get GO terms for LUSC genes
```{r}
library(limma)

# Over-representation analyses for GO terms for genes highly expressed in LUSC
go_LUSC <- goana(up_LUSC_ID, species="Hs")
topGO(go_LUSC, ontology = "BP", number=10)
```

## Same thing for LUAD
```{r}
# Over-representation analyses for GO terms for genes highly expressed in LUAD
go_LUAD <- goana(up_LUAD_ID, species="Hs")
topGO(go_LUAD, ontology = "BP")
```

## Get KEGG pathways for LUSC
```{r}
# Over-representation analyses for KEGG pathways
kegg_LUSC <- kegga(up_LUSC_ID, species="Hs")
topKEGG(kegg_LUSC)
```
...
```{r}
# Over-representation analyses for KEGG pathways
kegg_LUAD <- kegga(up_LUAD_ID, species="Hs")
topKEGG(kegg_LUAD)
```

## ORA Results
- LUSC DE genes are enriched with epithelial/epidermis related GO terms
- These features are typical of squamous cell biology, supporting the validity of results
- Keratinization is a common pathological feature of LUSC tumors, further reinforcing the relevance of these pathways (PMCID: PMC10092975).
- LUAD DE genes are enriched with more metabolic and transportation related processes
- This high expression of these processes can be understood as metabolic reprogramming that supports rapid tumor growth and proliferation.
- The cause for differential expression of these traits may also be due to cell-of-origin characteristics or the specific metabolic requirements of the LUAD tumor
- Further investigation on this is needed
...
## Preprocess entire list of DE genes for GSEA
```{r}
#Preprocess gene_id
deg$ensembl_gene_id <- sub("\\..*", "", rownames(deg))
...
# Keep only genes with an entrez ID
deg_annot <- deg_annot[!is.na(deg_annot$entrezgene_id), ]

# Remove duplicates. Keep max(|log2FoldChange|)
library(dplyr)
deg_annot <- deg_annot |>
...
```{r}
# Rank all DE genes by log2FoldChange
gene_ranks <- deg_annot$log2FoldChange
names(gene_ranks) <- deg_annot$entrezgene_id

# Create sorted vector of entrez IDs with all genes (LUSC will be at the top)
gene_ranks <- sort(gene_ranks, decreasing = TRUE)
```

## Install package for GSEA
```{r}
...
# Get GSEA results with significant p-value
sig_results_gsea <- gsea@result[gsea@result$p.adjust < 0.05, ]

# Split results based on LUSC vs. LUAD
lusc_enriched  <- sig_results_gsea[sig_results_gsea$NES > 0, ]
luad_enriched <- sig_results_gsea[sig_results_gsea$NES < 0, ]
```

## Enrichment curves
```{r}
# Enrichment curves for top LUSC enriched pathways
gseaplot2(
  gsea,
  geneSetID = lusc_enriched$ID[1],
  title = lusc_enriched$Description[1]
)
gseaplot2(
  gsea,
  geneSetID = lusc_enriched$ID[2],
  title = lusc_enriched$Description[2]
)
gseaplot2(
  gsea,
  geneSetID = lusc_enriched$ID[3],
  title = lusc_enriched$Description[3]
)

# Enrichment curves for top LUAD enriched pathways
gseaplot2(
  gsea,
  geneSetID = luad_enriched$ID[1],
  title = luad_enriched$Description[1]
)
gseaplot2(
  gsea,
  geneSetID = luad_enriched$ID[2],
  title = luad_enriched$Description[2]
)
gseaplot2(
  gsea,
  geneSetID = luad_enriched$ID[3],
  title = luad_enriched$Description[3]
)
```

## GSEA Results
- Similar to pathways identified in ORA
- LUSC DE genes are enriched with epithelial/epidermis related terms
- LUAD DE genes are enriched with more metabolic and transportation related processes
